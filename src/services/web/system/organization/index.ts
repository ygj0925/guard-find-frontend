...
export async function query(body?: Partial<SysOrganizationQo>) {
  return request<R<SysOrganizationVo[]>>('system/dept/tree', {
    method: 'GET',
    params: body,
  });
}
...
export async function create(body: SysOrganizationDto) {
  return request<R<any>>('system/dept', {
    method: 'POST',
    data: body,
  });
}
...
export async function del(body: SysOrganizationVo) {
  return request<R<any>>(`system/dept/${body.id}`, {
    method: 'DELETE',
  });
}

export function revised() {
  return request('/system/dept/revised', {
    method: 'patch',
  });
}
